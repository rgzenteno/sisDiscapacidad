...
/**
 * Reporte "Bajas" (tab "Bajas" de Bandeja de Pagos), en Excel. Mismo modelo
 * que NoPagadosExport.php: título institucional que varía según el filtro
 * "Tipo de Baja" (todos / temporal / definitiva), tabla N°/C.I./Nombre/Grado
 * de Discapacidad/Monto No Pagado (mismas columnas que No Pagados, sin
 * Observaciones), y un total al final.
 */
class BajasExport implements FromArray, WithEvents, WithTitle, WithColumnWidths
{
    private const MESES = [
    ...
    private function sufijoBaja(): string
    {
        return match ($this->estadoBaja) {
            'baja_temporal' => 'DE LOS FUNCIONARIOS',
            'baja_definitiva' => 'QUE FALLECIERON',
            default => 'CON BAJAS',
        };
    }
    ...
    {
        return 'AL MES DE ' . mb_strtoupper($this->nombreMes()) . " GESTION {$this->gestion}";
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $tituloLinea2 = 'GRAVE MUY GRAVE ' . $this->sufijoBaja() . ' ' . $this->periodoTexto()
                    . ($this->verRetro ? ' (RETROACTIVOS)' : '');
                ...
                $sheet->setCellValue("A{$fila}", 'TOTAL NO PAGADOS ' . $this->periodoTexto());
