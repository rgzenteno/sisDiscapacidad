...
class LogService
{
    ...
    public function logBoletaDescarga($pago, int $idPago, string $gestion, string $mes, ?string $beneficiario = null, ?string $description = null)
    {
        return $this->logFull(
            'boleta_descargada',
            'BandejaPago',
            $description ?? "Boleta descargada del beneficiario {$beneficiario} del pago de {$mes} - {$gestion}.",
            $pago,
            [
                'id_pago'      => $idPago,
                'gestion'      => $gestion,
    ...
    public function logCreation(string $module, $subject, ?string $description = null, $affectedUser = null, array $campos = [], array $presupuestoAjustes = [])
    {
        $extraData = [
            'details' => [
                'before' => null,
        ...
        if (!empty($presupuestoAjustes)) {
            $extraData['presupuesto_ajustes'] = $presupuestoAjustes;
        }
        ...
        return $this->logFull(
            ...
            [
                'details' => [
                    ...
                    // Ya se calculaban en GestionController::addMes() pero se
                    // descartaban acá antes de llegar a la Bitácora — el
                    // frontend (ModalLogs.vue) ya tenía el HTML listo para
                    // mostrarlos, solo faltaba que el backend los pasara.
                    'bajas_temporales_omitidas' => $resumen['bajas_temporales_omitidas'] ?? null,
                    'bajas_definitivas_omitidas' => $resumen['bajas_definitivas_omitidas'] ?? null,
                    'registros_depurados'       => $resumen['registros_depurados'] ?? null,
                    'fallecidos_detectados'     => $resumen['fallecidos_detectados'] ?? null,
                    ...
                    'fallecidos'                => $resumen['fallecidos'] ?? [],
                    // Lista completa (ci, nombre, motivo) — permite revisar
                    // en cualquier momento posterior, ej. conflictos de CI
                    // con nombre distinto detectados en el PDF, sin depender
                    // de que alguien lo haya anotado al momento de la carga.
    ...
    public function logUpdate($module, $model, $changes = null, $description = null, array $presupuestoAjustes = [])
    {
        $extraData = [];
    ...
    public function logDeletion($module, $model, $description, array $campos = [], array $presupuestoAjustes = [])
    {
        $extraData = [];
