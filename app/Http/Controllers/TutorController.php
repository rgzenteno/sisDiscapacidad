...
class TutorController extends Controller
{
    ...
    {
        // ── FLUJO: beneficiario es su propio tutor ──────────────────────────────
        if ($request->boolean('es_propio')) {

            // Asignación posterior (bene ya registrado)
            if ($request->filled('id_persona')) {
                $persona = Persona::findOrFail($request->id_persona);
                $persona->tutor_nombre = 'propio';
                $persona->save();
                ...
                $this->logService->logUpdate(
                    ...
                    $persona,
                    [
                        'campos_modificados' => ['tutor' => 'propio'],
                        'valores_anteriores' => ['tutor' => null],
                        'valores_nuevos'     => ['tutor' => 'propio'],
                    ],
                    "Se marcó al beneficiario {$nombre} como su propio tutor."
                );
            ...
            // Flujo creación nuevo bene: guarda en sesión para PersonaController@store
            session(['tutor_propio' => true]);
            session()->forget('selected_tutor_id');
            return back();
        }

        // ── FLUJO NORMAL: tutor con datos ───────────────────────────────────────
        $data = $request->all();
        ...
        if ($existingTutor) {
            ...
            if (!empty($cambios)) {
                ...
                $camposModificados = $valoresAnteriores = $valoresNuevos = [];
