...
class HabilitadoController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        $selectedYear = request()->input('año', session('habilitado_year', Carbon::now()->year));
    ...
    /* public function showHabilitado(string $encrypted_id)
    {
        // Desencriptar el ID
        try {
        ...
        // Query para datos habilitados con verificación de estado y fecha de registro
        $datosHabilitado = DB::table('gestion as g')
            // JOINs tradicionales - más claros
            ->leftJoin('mes as m', 'g.id_gestion', '=', 'm.id_gestion')
            ->leftJoin('persona as p', 'p.id_persona', '=', DB::raw($id_persona))
            ->leftJoin('habilitado as h', function ($join) use ($id_persona) {
                $join->on('g.id_gestion', '=', 'h.id_gestion')
                    ->on('m.id_mes', '=', 'h.id_mes')
                    ->where('h.id_persona', '=', $id_persona);
            })
            ->leftJoin('pago', 'h.id_habilitado', '=', 'pago.id_habilitado')

            /* ->leftJoin('historial_estados as he', function ($join) use ($id_persona) {
                $join->where('he.id_persona', '=', $id_persona)
                    ->whereRaw("he.fecha_inicio <= LAST_DAY(CONCAT(g.gestion, '-', LPAD(m.mes, 2, '0'), '-01'))")
                    ->where(function ($subJoin) {
                        $subJoin->whereNull('he.fecha_fin')
                            ->orWhereRaw("he.fecha_fin >= CONCAT(g.gestion, '-', LPAD(m.mes, 2, '0'), '-01')");
                    });
            })
            ...
            // Filtros principales más legibles
            ->where('p.id_persona', $id_persona) // Asegurar que obtenemos datos de la persona correcta
            ->whereRaw("CONCAT(g.gestion, '-', LPAD(m.mes, 2, '0')) >= DATE_FORMAT(p.fecha_registro, '%Y-%m')")

            // Condición de baja definitiva simplificada
            ->where(function ($query) use ($id_persona) {
                $query->whereNotExists(function ($subQuery) use ($id_persona) {
                    $subQuery->select(DB::raw(1))
                        ->from('historial_estados as he_baja')
                        ->where('he_baja.id_persona', $id_persona)
                        ->where('he_baja.estado', 'baja_definitiva')
                        ->whereNull('he_baja.fecha_fin');
                })
                    ->orWhereRaw("
                                CONCAT(g.gestion, '-', LPAD(m.mes, 2, '0')) <= DATE_FORMAT((
                                    SELECT he_baja.fecha_inicio
                                    FROM historial_estados he_baja
                                    WHERE he_baja.id_persona = ?
                                    AND he_baja.estado = 'baja_definitiva'
                                    AND he_baja.fecha_fin IS NULL
                                    LIMIT 1
                                ), '%Y-%m')
                            ", [$id_persona]);
            })
            ...
            ->select([
                ...
                'm.mes',
                'm.monto',
                DB::raw('MAX(h.id_habilitado) as id_habilitado'),
                DB::raw('MAX(h.fecha_habilitado) as fecha_habilitado'),
                DB::raw('MAX(h.observaciones_habilitado) as observaciones_habilitado'),
                DB::raw('MAX(h.habilitado) as habilitado'),
                DB::raw('MAX(h.created_at) as created_at'),
                DB::raw('(SELECT he2.estado FROM historial_estados he2
                                WHERE he2.id_persona = p.id_persona
                                AND he2.fecha_inicio <= LAST_DAY(CONCAT(g.gestion, "-", LPAD(m.mes, 2, "0"), "-01"))
                                ORDER BY he2.fecha_inicio DESC
                                LIMIT 1) as estado_mes'),
                DB::raw('MAX(he.id) as estado_actual'),
                DB::raw('MAX(he.observaciones) as observacion'),
                DB::raw('CASE WHEN pago.id_pago IS NOT NULL THEN 1 ELSE 0 END as tiene_pago'),
                'p.fecha_registro'
            ])
            ->groupBy([
                'p.id_persona',
        ...
        $datosRecibo = DB::table('Persona as p')
            ->join('tutor as t', 'p.id_tutor', '=', 't.id_tutor')
        ...
        // Condición base para la consulta de habilitados
        $query = $id_persona
            ? Habilitado::where('id_persona', $id_persona)
            : Habilitado::query();

        return Inertia::render('Habilitados/listaHabilitados', [
            'totalHabilitado' => $datosHabilitado->total(),
            'datosHabilitado' => $datosHabilitado,
