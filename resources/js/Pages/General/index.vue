...
const handleImportar = (archivo, limpiarArchivo) => {
    ...
    router.post(route('general.importar.store'), formData, {
        preserveScroll: true,
        onSuccess: () => {
            // Ya no necesitas validar el formato aquí,
            // porque ya se validó en el frontend
...
<template>
    ...
    <div class="flex h-screen bg-gray-200 font-roboto">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <Form v-if="formCreateTutor" :fields="tutorFields" :botonOmitir="true" :idFor="selectedId" :tutores="tutor"
                ...
                @omitir="openModalEditBeneficiary">
                <template #icon>
                    <svg class="w-5 h-5 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="currentColor" viewBox="0 0 24 24">
                        <path fill-rule="evenodd"
                ...
                <template #label2>
                    Asigne un tutor al beneficiario
                </template>
            </Form>
            ...
            <ModalEstados v-if="showModalRegistrado" :tipo="modalData.tipo" :registrado-id="modalData.registradoId"
                ...
                <ModalImportaciones
        v-if="mostrarModal"
        :titulo="configPlanillaGeneral.titulo"
        :subtitulo="configPlanillaGeneral.subtitulo"
        :icono="configPlanillaGeneral.icono"
        ...
        :textoBotonImportar="configPlanillaGeneral.textoBotonImportar"
    ...
    />
            ...
            <div :class="general.data.length <= 15 ? 'mt-0.5' : 'mt-0'">
                <Paginacion v-if="general?.last_page > 1" :links="general.links" :from="general.from"
                    :to="general.to" :total="general.total" />
                <Footer />
            </div>
        </div>
    </div>
