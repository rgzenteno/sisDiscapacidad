...
const tableColumns = [
    ...
    { label: 'Monto a Pagar (bs.)', headerClass: 'text-center', cellClass: 'whitespace-nowrap' },
    ...
    <div class="flex h-screen bg-gray-50 font-roboto">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                                                    ...
                                                    <li v-for="year in gestiones" :key="year.anio">
                                                        <a href="#" @click.prevent="selectGestion(year)"
                                                            ...
                                                            <svg v-if="selectedYear && selectedYear.toString() === year.anio.toString()"
                                                                class="w-4 h-4 text-[#13326A] shrink-0"
                                                                fill="currentColor" viewBox="0 0 20 20">
                                                                <path fill-rule="evenodd"
            ...
            <DataTable :data="resultados.data" :columns="tableColumns" row-key="id_habilitado" :row-class="(item) => ({
                'bg-white font-bold text-black hover:bg-gray-100': item.estado_actual.estado === 'activo',
                '!bg-blue-800 font-medium !text-blue-200 hover:!bg-blue-700': item.estado_actual.estado === 'baja_temporal',
                'bg-yellow-300 font-bold text-red-600 hover:bg-yellow-400': item.estado_actual.estado === 'baja_definitiva'
            })">

                <template #row="{ item, index }">

                    <!-- Columna: Nº -->
                    <td class="text-center px-2 py-1.5 whitespace-nowrap">
                    ...
                    <!-- Columna: Apellidos y nombres P.C.D. -->
                    <td class="px-2 py-1.5 uppercase whitespace-nowrap">
                        <p v-if="item.nombre">{{ item.apellido }} {{ item.nombre }}</p>
                    ...
                    <td class="text-center px-2 py-1.5 whitespace-nowrap">
                        <span v-if="item.estado_actual.estado !== 'activo'" class="uppercase">
                            {{ item.observaciones }}
                        </span>
                    </td>
