...
const carnetFields = [
    ...
    },
    {
        typeInput: 'text',
        name: 'doc',
        label: 'Número de Carnet Discapacidad',
        type: 'text',
        readonly: true,
...
const tableColumns = [
    ...
    { label: 'Carnet de Discapacidad', field: 'carnet', headerClass: 'text-center', cellClass: 'whitespace-nowrap' },
...
/**
 * Abre el formulario para asignar tutor a un postulante
 * @param {number|string} idPersona - ID de la persona
 */
const openCreateTutor = (idPersona) => {
    const postulante = persona.value.data.find(p => p.id_persona === idPersona);

    if (!postulante) {
        mostrarMensaje('error', 'Error', 'No se encontró el postulante');
        return;
    }

    const tutorSeparado = separarNombre(postulante.tutor_nombre);

    selectedItem.value = {
        nombre_tutor: tutorSeparado.nombre,
        apellido_tutor: tutorSeparado.apellido,
        tiene_tutor: !!postulante.tutor_nombre
    };

    selectedId.value = idPersona;
    addTutor.value = true;
    createTutor.value = true;
};
...
/**
 * Abre el formulario de opciones (distrito)
 */
const openOption = () => {
...
/**
 * Abre el formulario de opciones (discapacidad)
 */
const openOptionDis = () => {
    formCreateOptionDis.value = true;
};
    ...
    <div class="flex h-screen bg-gray-200 font-roboto">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <Form v-if="formCreate" :fields="personaFields" :distritos="distrito" :nombreFor="String(selectedTutorName)"
                submit-route="persona.store" @add="handleAddBene" @openFormOption="openOption" @sinDatos="sinDatos"
                @cancel="closeForm" @close="encotrado">
                <template #icon>
            ...
            <Form v-if="formEdit" :fields="personaFieldsEdit" boton-name="Guardar" :distritos="distrito" :idFor="selectedId"
                :existing-data="selectedItem || {}" :edit-mode="true" submit-route="persona.update" @add="handleEdit"
                @openFormOption="openOption" @sinDatos="sinDatos" @cancel="closeForm" @close="closeForm">
                <template #icon>
            ...
            <Form v-if="formCreateTutor" :fields="tutorFields" :botonOmitir="true" :idFor="selectedId" :tutores="tutor"
                submit-route="tutor.store" @add="handleAddTutor" @sinDatos="sinDatos"
                @tutorEncontradoSms="smsTutorEncontrado" @encontrado="encontrado" @cancel="closeForm" @omitir="omitir">
                <template #icon>
            ...
            <Form v-if="createTutor" :fields="tutorFields" :idFor="selectedId" :existing-data="selectedItem || {}"
                :tutores="tutor" :addTutor="addTutor" submit-route="tutor.store" @add="handleTutor"
                @tutorEncontradoSms="smsTutorEncontrado" @sinDatos="sinDatos" @cancel="closeForm">
                <template #icon>
                    <Icon :icon-button="true" name="users" class-name="text-white" />
                </template>
                <template #label1>
            ...
            <Form v-if="formCreateCarnet" :fields="carnetFields" :discapacidad="discapacidad" :idFor="selectedId"
                ...
                submit-route="carnet.store" @add="addCarnet" @openFormOption="openOptionDis" @sinDatos="sinDatos"
                @cancel="closeForm">
                <template #icon>
            ...
            <Form v-if="showModalCarnetEdit" :fields="carnetFieldsEdit" :discapacidad="discapacidad"
                :existing-data="selectedItem || {}" :nombreFor="selectedNombre" :idFor="selectedId" :edit-mode="true"
                submit-route="carnet.update" @add="handleEditCarnet" @openFormOption="openOptionDis"
            ...
            <Form v-if="formCreateOption" :distritos="distrito" :fields="DistritoFields" submit-route="dropdown.store"
                @add="handleAddDis" @encontrado="mensajeExisteDis" @cancel="closeFormDis" @close="closeFormDis">
                <template #icon>
            ...
            <Form v-if="formCreateOptionDis" :discapacidad="discapacidad" :fields="DiscapacidadFields"
                submit-route="dropdown.addDis" @add="handleAddDis" @encontrado="mensajeExisteDisca" @cancel="closeFormDis"
                @close="closeFormDis">
                <template #icon>
                    <Icon :icon-button="true" name="clipboard" class-name="text-white" />
                </template>
            ...
            <ModalCarnet v-if="showModalCarnet" :data="selectedItem" @edit="openEditCarnet" @close="closeForm" />

            <!-- Modal: Estados del Importacion -->
            ...
            <div class="flex justify-between p-4 pb-3 bg-gray-50 border-x-2 border-t-2 rounded-t-lg mr-1">
                ...
                <div class="flex gap-2">
                    <!-- Botón: Agregar -->
                    <Button v-if="can('crear-beneficiario')" id="btn-agregar" @click.prevent="openTutor()"
                ...
                <template #row="{ item }">
                    ...
                    <td class="px-3 py-1 whitespace-nowrap">
                        <div class="flex items-center">
                            <div @click.prevent="item.carnet?.id_carnet ? getCarnetUrl(item) : (can('crearCarnet-beneficiario') ? (!item.fecha_nacimiento ? openMissingDataModal() : openCreateCarnet(item.id_persona, `${item.nombre_persona} ${item.apellido_persona}`, `${item.fecha_nacimiento}`)) : null)"
                                :class="[
                                    'flex items-center gap-1',
                                    item.carnet?.id_carnet ? 'cursor-pointer action-link' : (can('crearCarnet-beneficiario') ? 'cursor-pointer action-link' : 'cursor-not-allowed'),
                    ...
                    <td class="px-1 py-1 whitespace-nowrap">
                        <div class="flex items-center">
                            <div @click.prevent="item.tutor?.id_tutor ? ModalTutorDatos(item) : (can('asignaTutor-beneficiario') ? openCreateTutor(item.id_persona) : null)"
                                :class="[
                                    'flex items-center gap-1',
                                    item.tutor?.id_tutor ? 'cursor-pointer action-link' : (can('asignaTutor-beneficiario') ? 'cursor-pointer action-link' : 'cursor-not-allowed'),
