...
const tableColumns = [
    ...
    { label: 'Usuario', field: 'causer', headerClass: 'text-center', cellClass: 'whitespace-nowrap' },
    { label: 'Fecha y Hora', field: 'created_at', headerClass: 'whitespace-nowrap', cellClass: 'whitespace-nowrap' },
    { label: 'Módulo', field: 'properties.module', headerClass: 'text-center', cellClass: 'whitespace-nowrap' },
...
const hoverBg = {
    ...
    'boleta_impresa': '#f0fdf4',
    'pago anulado': '#fef2f2',
    'pago reactivado': '#f0fdf4',
};
...
const actionConfig = {
    ...
    'reset contraseña': { label: 'Reset', icon: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z', classes: 'bg-violet-50 text-violet-700 ring-violet-200', dot: 'bg-violet-400' },
    ...
    'boleta_descargada': { label: 'Boleta descargada', icon: 'M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2m-8 1V4m0 12-4-4m4 4 4-4', classes: 'bg-teal-50 text-teal-700 ring-teal-200', dot: 'bg-teal-400' },
    ...
    'boleta_impresa': { label: 'Boleta Impresa', icon: 'M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z', classes: 'bg-teal-50 text-teal-700 ring-teal-200', dot: 'bg-teal-400' },
};
...
<template>
    <AppLayout>
        ...
        <div class="flex justify-between p-2 sm:p-4 sm:pb-3 bg-gray-50 border-x-2 border-t-2 rounded-t-lg mr-1">
            <!-- Buscador -->
            <Busqueda :initial-value="filters.buscador" name="log" only="logs" :data="logs" ruta-busqueda="log.index" />
        </div>

        <!-- ============================================================================ -->
        <!-- TABLA DE DATOS -->
        <!-- ============================================================================ -->
        <DataTable :data="logs.data" :columns="tableColumns" row-key="id" empty-message="No hay registros de actividad."
            :on-row-click="(item) => modalLogs(item.id, item)" :row-hover-style="(item) => ({
            ...
            <template #row="{ item }">
                ...
                <td class="px-2 py-2.5 text-center whitespace-nowrap">
                    <span
                        class="inline-flex items-center justify-center gap-0.5 w-32 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset capitalize"
