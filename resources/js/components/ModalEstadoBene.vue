...
// Emits
const emit = defineEmits(['add', 'close', 'cambioEstado', 'addEstado', 'delete', 'editEstado']);
...
const historialEstados = computed(() => {
    if (props.data.historial_completo && Array.isArray(props.data.historial_completo)) {
        return props.data.historial_completo.map((registro, index, arr) => {
            // index 0 = más reciente → "En curso"
            // index i > 0 → hasta = un mes antes del inicio del estado más reciente (arr[i-1])
            const hasta = index === 0 ? 'Actual' : restarUnMes(arr[index - 1].fecha_inicio);
...
const estadoActual = computed(() => {
    if (historialEstados.value.length > 0) {
        const ultimo = historialEstados.value[0];
        return {
...
const getIconoEstado = (estado) => {
    ...
    switch (estado.toLowerCase()) {
        ...
        case 'depurado':
            return 'h-2.5 w-2.5 rounded-full ring-2 ring-gray-300 bg-gray-500'
        default:
            return '●'
    }
}
...
<template>
    ...
    <!-- Overlay del modal -->
    <div class="fixed inset-0 bg-slate-900/75 backdrop-blur-sm flex items-center justify-center z-30 p-4"
        @click="cerrarModal">
        <!-- Modal container -->
        <div class="relative w-full max-w-2xl max-h-[95vh] bg-gradient-to-br from-white to-slate-50 rounded-3xl shadow-[0_10px_40px_rgba(15,23,42,0.06)] border border-gray-100 flex flex-col overflow-hidden"
            @click.stop>
            <!-- Header -->
            <div class="grid grid-cols-[1fr_auto] gap-6 px-2 sm:px-6 py-4 border-b border-gray-100">
                <!-- Contenido principal -->
                <div class="min-w-0">
                    ...
                    <div class="grid grid-cols-[auto_1fr_auto] gap-4 items-start mb-3">
                        ...
                        <!-- Título y subtítulo -->
                        <div class="min-w-0">
                            <h2 class="text-md sm:text-2xl font-semibold text-slate-800 tracking-tight truncate">Gestión
                                de
                                Estados
                            </h2>
                            <h6 class="text-xs flex sm:text-sm text-slate-500 capitalize tracking-tight truncate">
                                {{ toTitleCase(nombreDisplay) }} • {{ ciCompleto }}
                            </h6>
                        </div>
                        ...
                        <div class="hidden sm:block flex-shrink-0">
                            <span
                                ...
                                <svg v-if="estadoActual.estado === 'depurado'" class="w-4 h-4 text-gray-500"
                    ...
                    <div class="flex flex-wrap gap-2 text-xs sm:ms-2 text-slate-500">
                        <!-- Último cambio -->
                        <div
            ...
            <div class="flex-1 sm:px-6 py-4 min-h-0">
                <div class=" h-full">

                    <!-- Left: Timeline / historial -->
                    <div class="xl:col-span-1 px-2">
                        ...
                        <div class="relative pl-2 sm:pl-6 flex flex-col flex-1 mx-auto sm:w-10/12">
                                    ...
                                    <div class="relative flex items-start gap-4">
                                        ...
                                        <div class="absolute left-[-6px] top-2">
                                            <span class="block w-3 h-3 rounded-full ring-2 ring-white shadow-sm" :class="{
                                                ...
                                                'bg-gray-400': registro.estado === 'depurado',
                                            ...
                                            <div
                                                    ...
                                                    <div class="flex items-center gap-3">
                                                        <div class="flex flex-col">
                                                            ...
                                                            <span class="text-xs text-slate-400">
                                                                ...
                                                                <span v-if="registro.desde != registro.hasta">
                                                                    <span
                                                                        v-if="registro.hasta !== 'Actual' && registro.hasta && formatearMes(registro.desde) !== formatearMes(registro.hasta)">
                                                                        → {{ formatearMes(registro.hasta) }}
                                                                    </span>
                                                                    <span v-else-if="registro.hasta === 'Actual'"
                                                ...
                                                <div
                                                    ...
                                                    <div class="min-w-[90px] text-right">
                                                        <button
                                                            v-if="index === 0 && historialEstados.length > 1 && can('eliminar-estado')"
                                                            @click="emit('delete', registro.id)"
                                                            title="Eliminar registro"
                                                            class="inline-flex items-center gap-1 px-2 py-2 sm:py-1 text-xs text-red-600 bg-red-200 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
                                                        ...
                                                        <button v-else-if="index === 0 && can('superusuario')"
                                                            @click="emit('editEstado', registro)" title="Editar estado"
                                                            class="inline-flex items-center gap-1 px-2 py-1 text-xs text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors">
                                                            <Icon :icon-button="true" name="edit"
                                                                class-name="text-blue-600" :size="20" />
                                                            <span>Editar</span>
                                                        </button>

                                                        <!-- <div v-else-if="index === 0 && can('superusuario') && props.data.meses_pagados?.length"
                                                            class="relative group inline-flex items-center gap-1 px-2 py-1 text-xs text-amber-600 bg-amber-50 rounded-lg cursor-default"
                                                            title="">
                                                            ...
                                                            <span class="hidden sm:block">No editable</span>

                                                            <div
                                                                class="absolute bottom-full right-0 mb-2 w-56 z-50 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-200 ease-out pointer-events-none">
                                                                <div
                                                                    class="bg-slate-800 text-white text-xs rounded-lg px-3 py-2 shadow-lg leading-relaxed">
                                                                    Este estado no puede editarse porque el beneficiario
                                                                    ya tiene pagos registrados en este estado.
                                                                    <div
                                                                        class="absolute bottom-[-4px] right-4 w-2 h-2 bg-slate-800 rotate-45">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div> -->
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-white">
                ...
                <Button type="button" class="text-slate-700 bg-white hover:bg-slate-100" @click="emit('close')">
                    Cerrar
                </Button>
