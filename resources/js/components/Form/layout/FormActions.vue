...
// ============ INICIO EMITS ============ //
const emit = defineEmits(['submit', 'cancel', 'omitir', 'reset']);
// ============ FIN EMITS ============ //
...
const isDisabled = computed(() => {
    return props.processing || props.importing || (props.editMode && !props.isDirty);
});
...
const containerClasses = computed(() => {
    if (props.fieldCount >= 6) {
        return 'border-t border-gray-200 dark:border-gray-600 pt-4 px-4 ms:px-6 pb-4 bg-white dark:bg-gray-800 rounded-b-3xl';
    }
...
<template>
    <div :class="containerClasses">
        <div class="flex items-center justify-end space-x-1 sm:space-x-3">
            ...
            <!-- Botón Reestablecer datos (editMode) -->
            <Button v-if="editMode && isDirty && !processing" type="button"
    class="text-orange-600 border-orange-300 bg-white hover:bg-orange-50" @click="emit('reset')">
    Restablecer
</Button>

            <!-- Botón Submit con Spinner -->
            <Button type="submit" class="text-white bg-blue-600 hover:bg-blue-500" :class="{ 'opacity-25': isDisabled }"
                :disabled="isDisabled">
