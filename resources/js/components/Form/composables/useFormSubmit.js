...
export function useFormSubmit(form, props, emit) {
    ...
    const handleSubmit = async (fileValidation = {}) => {
        ...
        if (campoDoc && form.doc) {
            const regexCarnet = /^03-\d{8}[A-ZÑ]{2,3}$/u;
        ...
        if (props.data.id_estado_base) {
            form.id_estado_base = props.data.id_estado_base;
        }

        if (props.data.fecha_inicio_forzada) {
            form.fecha_inicio = props.data.fecha_inicio_forzada;
        }
