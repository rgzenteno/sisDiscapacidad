...
const actionConfig = {
    ...
    'boleta_descargada': { label: 'Boleta Descargada', icon: 'M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2m-8 1V4m0 12-4-4m4 4 4-4', fondoIcon: 'bg-teal-50 dark:bg-teal-900/20', text: 'text-teal-600 dark:text-teal-400', badge: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300' },
    'pago anulado': { label: 'Pago Anulado', icon: 'm15 9-6 6m0-6 6 6m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z', classes: 'bg-red-50 text-red-700 ring-red-200', dot: 'bg-red-400', fondoIcon: 'bg-red-50', text: 'text-red-600', badge: 'bg-red-100 text-red-700' },
    'pago reactivado': { label: 'Pago Reactivado', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15', classes: 'bg-green-50 text-green-700 ring-green-200', dot: 'bg-green-400', fondoIcon: 'bg-green-50', text: 'text-green-600', badge: 'bg-green-100 text-green-700' },
    'boleta_impresa': { label: 'Boleta Impresa', icon: 'M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z', fondoIcon: 'bg-teal-50 dark:bg-teal-900/20', text: 'text-teal-600 dark:text-teal-400', badge: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300' },
};
...
<template>
    <Modal :showFooter="false" maxWidth="max-w-xl" @close="emit('close')"
        ...
        <div class="overflow-y-auto flex-1 px-0 py-1 space-y-4">
            ...
            <!-- BOLETA DESCARGADA -->
            <template
                v-if="props.data.properties.action === 'boleta_descargada' || props.data.properties.action === 'boleta_impresa'">
            ...
            <template v-if="props.data.properties.action === 'importar' && props.data.properties.details">
                ...
                <div v-if="
                    ...
                    props.data.properties.details.fallecidos_detectados != null
                " class="rounded-xl border border-indigo-100 dark:border-indigo-900/30 overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-px bg-gray-100 dark:bg-gray-700/30">
                        ...
                        <div v-if="props.data.properties.details.fallecidos_detectados != null"
                            class="bg-white dark:bg-gray-900 px-4 py-3">
                            <p class="text-xs text-gray-400 mb-0.5">Fallecidos detectados</p>
                            <p class="text-sm font-bold text-gray-600 dark:text-gray-300">
                                {{ props.data.properties.details.fallecidos_detectados }}
                            </p>
                        </div>
                ...
                <!-- Observaciones: casos que requieren revisión manual (ej. CI -->
                <!-- repetido en el PDF con nombres distintos) — quedan acá para -->
                <!-- poder revisarlos en cualquier momento, no solo al momento de la carga. -->
                <div v-if="props.data.properties.details.observaciones?.length"
                    class="rounded-xl border border-amber-100 dark:border-amber-900/30 overflow-hidden">
                    <div class="bg-amber-50 dark:bg-amber-900/20 px-4 py-2.5">
                        <p class="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wide">
                            Observaciones — revisar manualmente
                        </p>
                    </div>
                    <ul class="divide-y divide-gray-100 dark:divide-gray-700/30">
                        <li v-for="(obs, i) in props.data.properties.details.observaciones" :key="i"
                            class="bg-white dark:bg-gray-900 px-4 py-2.5">
                            <div class="flex items-center gap-2 mb-1">
                                <span
                                    class="text-[11px] font-mono px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">
                                    CI: {{ obs.ci }}
                                </span>
                                <span v-if="obs.nombre" class="text-xs font-medium text-gray-700 dark:text-gray-300">
                                    {{ obs.nombre }}
                                </span>
                            </div>
                            <p class="text-xs text-amber-700 dark:text-amber-400">{{ obs.motivo }}</p>
                        </li>
                    </ul>
                </div>
    ...
    {{ JSON.stringify(props.data.properties, null, 2) }}
            ...
            <template
                v-if="['habilitado', 'deshabilitado', 'pago realizado', 'pago anulado', 'pago reactivado'].includes(props.data.properties.action) && props.data.properties.detalles">
                <div class="rounded-xl border border-gray-100 dark:border-gray-700/30 overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 gap-px bg-gray-100 dark:bg-gray-700/30">
                        ...
                        <div v-if="props.data.properties.detalles.presupuesto_anterior != null"
                            class="col-span-2 bg-white dark:bg-gray-900 px-4 py-3">
                            <p class="text-xs text-gray-400 mb-1.5">Presupuesto del mes</p>
                            <div class="flex items-center gap-3">
                                <div class="flex-1 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2">
                                    <p class="text-xs text-red-400 mb-0.5">Anterior</p>
                                    <p class="text-sm text-red-700 dark:text-red-300 font-medium">
                                        Bs. {{ Number(props.data.properties.detalles.presupuesto_anterior).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                    </p>
                                </div>
                                <svg class="w-4 h-4 text-gray-300 shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg px-3 py-2">
                                    <p class="text-xs text-emerald-400 mb-0.5">Nuevo</p>
                                    <p class="text-sm text-emerald-700 dark:text-emerald-300 font-medium">
                                        Bs. {{ Number(props.data.properties.detalles.presupuesto_nuevo).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <!-- PRESUPUESTO AJUSTADO EN CASCADA (una baja/reactivación de Estado puede afectar varios meses a la vez) -->
            <template v-if="props.data.properties.presupuesto_ajustes?.length">
                <div class="rounded-xl border border-amber-100 dark:border-amber-900/30 overflow-hidden">
                    <div class="bg-amber-50 dark:bg-amber-900/20 px-4 py-2.5">
                        <p class="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wide">
                            Presupuesto ajustado ({{ props.data.properties.presupuesto_ajustes.length }}
                            mes{{ props.data.properties.presupuesto_ajustes.length === 1 ? '' : 'es' }})
                        </p>
                    </div>
                    <div class="divide-y divide-gray-100 dark:divide-gray-700/30">
                        <div v-for="(ajuste, idx) in props.data.properties.presupuesto_ajustes" :key="idx"
                            class="px-4 py-3 bg-white dark:bg-gray-900">
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">{{ ajuste.mes }}</p>
                            <div class="flex items-center gap-3">
                                <div class="flex-1 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2">
                                    <p class="text-xs text-red-400 mb-0.5">Anterior</p>
                                    <p class="text-sm text-red-700 dark:text-red-300 font-medium">
                                        Bs. {{ Number(ajuste.anterior).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                    </p>
                                </div>
                                <svg class="w-4 h-4 text-gray-300 shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg px-3 py-2">
                                    <p class="text-xs text-emerald-400 mb-0.5">Nuevo</p>
                                    <p class="text-sm text-emerald-700 dark:text-emerald-300 font-medium">
                                        Bs. {{ Number(ajuste.nuevo).toLocaleString('es-BO', { minimumFractionDigits: 2 }) }}
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        <template #footer>
            <div class="px-5 py-3.5 border-t border-gray-100 dark:border-gray-700/50 flex justify-end">
                <button @click="emit('close')"
                    class="px-12 py-2.5 rounded-xl text-sm font-medium text-white bg-[rgb(var(--brand-600))] hover:bg-[rgb(var(--brand-500))]">
                    Cerrar
                </button>
            </div>
        </template>
