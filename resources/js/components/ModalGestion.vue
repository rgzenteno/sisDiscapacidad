...
const handleGenerarReporte = () => {
  ...
  const personas = Array.isArray(props.data.personas_pagadas)
    ? props.data.personas_pagadas
    : [];
  generarReportePagados(
    personas,
    props.data.gestion,
    props.data.mes,
    "Pagados",
...
const esUltimoMes = computed(() => {
  if (!props.ultimo_mes_id || !props.data?.id_mes) return false;
  return props.data.id_mes === props.ultimo_mes_id;
});
...
const getTotalBeneficiarios = computed(() => {
  return (
    Number(props.data.personas_activos || 0) +
...
const tieneAcciones = computed(() => {
  return (
    props.data.personas_sin_tutor > 0 ||
...
<template>
    ...
    <Modal :showHeader="true" :showFooter="false" maxWidth="max-w-4xl"
        :fondoIcon="esUltimoMes ? 'bg-blue-200' : 'bg-gray-200'" @close="$emit('close')">
        <template #icon>
            <svg class="w-6 h-6" :class="esUltimoMes ? 'text-blue-600' : 'text-gray-500'" aria-hidden="true"
        ...
        <template #label1>
            <div class="flex items-center gap-2">
                <span :class="esUltimoMes ? 'text-blue-700 dark:text-white' : 'text-gray-500 dark:text-white'">
        ...
        <template #label2>{{ getMonthDateRange(props.data.mes) }}</template>

        <!-- Body -->
        <div class="p-0 space-y-3 sm:space-y-4">
            <!-- Resumen de Gestión -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-1 sm:gap-4">
                <!-- Monto Total -->
                <div
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="text-sm font-medium text-gray-800 dark:text-gray-200">Beneficiarios Pagados</h3>
                            <Icon :icon-button="true" name="dollar" class-name="text-blue-600 dark:text-gray-500"
                    ...
                    <div
                        ...
                        <div class="text-center">
                            <div class="text-sm font-medium text-gray-800 dark:text-gray-200 mb-3"
                                :class="esUltimoMes ? 'text-blue-700 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'">
                                {{ getFromDate(props.data.mes) }}
                            </div>
                            <div class="grid grid-cols-7 gap-1">
                                <template v-for="day in getDateFin()" :key="day">
                                    <div class="w-6 h-6 flex items-center justify-center text-xs rounded-full" :class="{
                                        'bg-blue-500 text-white': esUltimoMes && day === getCurrentDay(),
                                        'bg-gray-400 text-white': !esUltimoMes && day === getDateFin()[getDateFin().length - 1],
                                        'text-gray-400 dark:text-gray-500': parseInt(day) > (esUltimoMes ? getCurrentDay() : getDateFin()[getDateFin().length - 1]),
                                        'font-medium text-gray-700 dark:text-gray-300': parseInt(day) <= (esUltimoMes ? getCurrentDay() : getDateFin()[getDateFin().length - 1]) && !((esUltimoMes && day === getCurrentDay()) || (!esUltimoMes && day === getDateFin()[getDateFin().length - 1]))
                                    }">
                                        {{ day }}
                                    </div>
                                </template>
    ...
    <div v-else class="bg-emerald-50 border border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800 p-4 rounded-lg">
        <div class="flex items-start">
            ...
            <div class="bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700 p-4 rounded-lg shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-1 sm:gap-4">
                    <div
                        class="bg-gray-50 border border-gray-200 dark:bg-gray-700/30 dark:border-gray-600 p-4 rounded-lg">
                        <div class="space-y-3">
        ...
        <template #footer>
            <div
                class="sticky bottom-0 flex justify-center sm:justify-end items-center px-6 py-4 border-t border-gray-200 dark:border-gray-700 rounded-b-xl bg-gray-50 dark:bg-gray-800/50 backdrop-blur-sm">
                <div class="flex space-x-3 items-center">
                    <Button v-if="can('reporte-mes')" @click="handleGenerarReporte()" type="button"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-300 text-gray-700 hover:bg-gray-200">
                        <Icon :icon-button="true" name="filePDF" class-name="text-red-600 dark:text-gray-500"
                            :size="18" />
                        <span class="hidden sm:block">Generar</span>
                        Reporte
                    </Button>
                    <Button @click="$emit('close')"
                        class="flex items-center gap-2 px-6 py-3 rounded-xl border text-white bg-blue-600 hover:bg-blue-500">
                        Aceptar
                    </Button>
                </div>
            </div>
