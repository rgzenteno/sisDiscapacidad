...
export const useReportePagosIndividualPDF = () => {
    ...
    const C = {
        ...
        boleta: { x: 125, w: 22 },
        monto: { x: 147, w: 24 },
        fecha: { x: 171, w: PAGE_W - M_RIGHT - 171 },
    };
    ...
    const drawLogos = (pdf) => {
        ...
        try {
            pdf.addImage("/images/sacaba.png", "PNG", M_LEFT, 2, 32, 13);
        } catch (_) { }
        try {
            pdf.addImage(
                "/images/sigamos.png",
    ...
    const drawDataRow = (pdf, item, rowNum, y, isOdd) => {
        ...
        pdf.setFillColor(anulado ? 252 : (isOdd ? 245 : 255), anulado ? 165 : (isOdd ? 245 : 255), anulado ? 165 : (isOdd ? 245 : 255));
        pdf.setDrawColor(180, 180, 180);
        ...
        pdf.text(formatFecha(item.fecha_pago), C.fecha.x + C.fecha.w / 2, ty, {
            align: "center",
