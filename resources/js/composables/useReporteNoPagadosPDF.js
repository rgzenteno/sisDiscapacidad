...
export const useReporteNoPagadosPDF = () => {
    ...
    /** Mismo título institucional que useReportePagosTodosPDF.js, en su
     * variante "NO PAGADOS". */
    ...
    /** Fila de total al final — mismo criterio que Total Pagados
     * (PagosTodosExport.php / useReportePagosTodosPDF.js). */
    ...
    const generarReporte = (datos, gestion, mes, fechaDesde = "", fechaHasta = "", distrito = "", verRetro = false) => {
        try {
            ...
            if (y + ROW_H > maxY) nuevaPagina();
            const labelTotal = `TOTAL NO PAGADOS ${periodoTexto(gestion, mes, fechaDesde, fechaHasta)}`;
            ...
            pdf.save(`Reporte_NoPagados-${fechaArchivo()}.pdf`);
        } catch (error) {
            console.error(error);
