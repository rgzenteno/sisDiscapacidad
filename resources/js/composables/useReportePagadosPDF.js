...
export const useReportePagadosPDF = () => {
    ...
    const calcTotales = (arr, montoMes = 0) => {
        ...
        const monto = montoMes || parseFloat(pagados.find(i => i.monto_pago !== null)?.monto_pago || 0);
    ...
    const drawFooterRow = (pdf, y, label, monto, bgColor) => {
        ...
        pdf.rect(C.boleta.x, y, C.boleta.w, h, "FD"); // ← celda boleta
        ...
        pdf.text(monto, C.monto.x + C.monto.w / 2, y + h / 2 + 1, { align: "center" });
    ...
    const drawTotales = (pdf, totales, mes, gestion, y) => {
        ...
        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(0.2);
        pdf.rect(C.n.x, y, spanW, h, "FD");
        pdf.rect(C.boleta.x, y, C.boleta.w, h, "FD");
        pdf.rect(C.monto.x, y, C.monto.w, h, "FD");
        pdf.rect(C.obs.x, y, C.obs.w, h, "FD");
        ...
        pdf.setFont("helvetica", "bold");
        pdf.setTextColor(0, 0, 0);
        pdf.text("SUMA TOTAL (BAJA DEFINITIVA + BAJA TEMPORAL + PAGADOS + PENDIENTES)", C.n.x + 1.5, y + h / 2 + 1, { maxWidth: spanW - 3 });
        pdf.text(fmt2(sumaTotal), C.monto.x + C.monto.w / 2, y + h / 2 + 1, { align: "center" });
    };
