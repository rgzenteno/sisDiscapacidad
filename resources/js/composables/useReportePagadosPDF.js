...
export const useReportePagadosPDF = () => {
    ...
    const calcTotales = (arr, montoMes = 0) => {
        ...
        const activos = arr.filter((i) => i.estado_actual !== "baja_definitiva" && i.estado_actual !== "baja_temporal");
        const pagados = arr.filter((i) => i.numero_boleta !== null && i.numero_boleta !== "");
        const sinPago = arr.filter((i) =>
            i.estado_actual !== "baja_definitiva" &&
            i.estado_actual !== "baja_temporal" &&
            (!i.numero_boleta)
        );
        ...
        return {
            ...
            monto_pagados: pagados.reduce((s, i) => s + parseFloat(i.monto_pago || 0), 0),
    ...
    const drawDataRow = (pdf, item, rowNum, y, montoUnitario) => {
        ...
        const montoAMostrar = esPagado ? (item.monto_pago || montoUnitario) : montoUnitario;
        pdf.text(fmt2(montoAMostrar), C.monto.x + C.monto.w / 2, ty, { align: "center" });
    ...
    // ─── Función principal ────────────────────────────────────────────────────
    // datosRetro: null si el mes no tiene retroactivo cargado, o el array de
    // personas del mes-retro correspondiente (misma forma que `datos`) para
    // agregar, debajo del mes normal y en página aparte, su propia planilla.
    const generarReporte = async (datos, gestion, mes, tipoR = "Reporte", montoMes = 0, datosRetro = null, montoRetro = 0) => {
        try {
            const pdf = new jsPDF({
            ...
            const dibujarSeccion = (datosSeccion, montoSeccion, esRetro) => {
                // Un registro está "anulado" solo si tiene boleta Y pago_flag === 0
                const arr = (Array.isArray(datosSeccion) ? datosSeccion : []).filter((i) => {
                    const anulado = i.numero_boleta && Number(i.pago_flag) === 0;
                    return !anulado;
                });

                const totales = calcTotales(arr, montoSeccion);
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    y = drawDataRow(pdf, arr[i], i + 1, y, montoSeccion);
                }
            ...
            dibujarSeccion(datos, montoMes, false);

            // La sección de retro siempre arranca en página nueva (nuevaPagina
            // agrega página apenas pageNum > 0), separada del mes normal.
            if (datosRetro !== null) {
                dibujarSeccion(datosRetro, montoRetro, true);
            }
