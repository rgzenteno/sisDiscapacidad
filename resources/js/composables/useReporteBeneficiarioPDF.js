...
export const useReporteBeneficiarioPDF = () => {
    ...
    const fmt2 = (n) => {
        const entero = String(Math.round(parseFloat(n || 0))); // "302000"
        return entero.replace(/\B(?=(\d{3})+(?!\d))/g, "."); // "302.000"
    };
    ...
    // ─── Totales ─────────────────────────────────────────────────────────────
    // "activo" y "pagos_suspendidos" se tratan igual en toda la vista (misma
    // fila blanca, mismo estilo normal, cuentan como "a pagar") — si se
    // cuentan distinto acá, el total general (arr.length * monto) deja de
    // coincidir con la suma del desglose (activos + bajas).
    const calcTotales = (arr) => {
        const activos = arr.filter((i) => i.estado_periodo === 'activo' || i.estado_periodo === 'pagos_suspendidos');
        const bajasDefinitivas = arr.filter((i) => i.estado_periodo === 'baja_definitiva');
        const bajasTemporales = arr.filter((i) => i.estado_periodo === 'baja_temporal');

        const monto = parseFloat(arr[0]?.monto || 0);
    ...
    const drawPageHeader = (pdf, mes, gestion, esRetro = false) => {
        ...
        pdf.text(
            `GRAVE Y MUY GRAVE MES DE ${getMes(mes)} GESTIÓN ${gestion}${esRetro ? " (RETROACTIVO)" : ""}`,
            PAGE_W / 2,
            13,
            { align: "center", maxWidth: PAGE_W - M_LEFT - M_RIGHT - 56 },
        );
    ...
    const drawDataRow = (pdf, item, rowNum, y) => {
        const estado = item.estado_periodo;

        if (estado === "baja_temporal") {
            pdf.setFillColor(0, 32, 96);      // #002060 azul oscuro
            pdf.setTextColor(0, 176, 240);    // #00B0F0 azul claro
        } else if (estado === "baja_definitiva") {
            pdf.setFillColor(255, 255, 0);    // #FFFF00 amarillo
            pdf.setTextColor(255, 0, 0);      // #FF0000 rojo
        } else {
        ...
        pdf.setFontSize(6.5);
        pdf.setFont("helvetica", estado === "activo" || estado === "pagos_suspendidos" ? "normal" : "bold");
        ...
        const nombre = capitalizarNombre(
            nombreConcatenado || item.nombre_completo || ""
        );
    ...
    const drawTotales = (pdf, totales, mes, gestion, y) => {
        ...
        // Rect fusionado de monto para las dos bajas
        ...
        // Rect único de obs abarcando todas las filas de totales
        const hTotalObs = y + footerRowHeight(pdf, `TOTAL PERSONAS CON DISCAPACIDAD A PAGAR MES DE ${mesNombre} ${gestion} (${cantidad_activos} PERSONAS)`, C.n.w + C.ci.w + C.nombre.w + C.grado.w - 3) - yInicioObs;
        pdf.setFillColor(255, 255, 255);
        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(0.2);
    ...
    // ─── Función principal ────────────────────────────────────────────────────
    const generarReporte = async (datos, gestion, mes, tipoR = "Reporte", esRetro = false) => {
        try {
            ...
            const nuevaPagina = () => {
                ...
                pageNum++;
                y = M_TOP;
                drawPageHeader(pdf, mes, gestion, esRetro);
            ...
            pdf.save(`0 PAGO DE BONO MES DE ${getMes(mes)} ${gestion}${esRetro ? " RETROACTIVO" : ""}.pdf`);
        } catch (error) {
