...
export const useReporteBeneficiarioPDF = () => {
    ...
    const calcTotales = (arr) => {
        const activos = arr.filter((i) => i.estado_periodo === 'activo');
    ...
    const drawDataRow = (pdf, item, rowNum, y) => {
        ...
        pdf.setFontSize(6.5);
        pdf.setFont("helvetica", estado === "activo" ? "normal" : "bold");
    ...
    const generarReporte = async (datos, gestion, mes, tipoR = "Reporte") => {
        try {
            ...
            pdf.save(`0 PAGO DE BONO MES DE ${getMes(mes)} ${gestion}.pdf`);
        } catch (error) {
