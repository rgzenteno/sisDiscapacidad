...
const submitButtonText = computed(() => {
    // PRIORIDAD 1: Si está importando (esto debe verificarse PRIMERO)
    if (props.importing) {
        return props.importingText;
    }

    // PRIORIDAD 2: Si está procesando (otros casos)
    if (props.processing) {
        return 'Procesando...';
    }

    // PRIORIDAD 3: Si viene botonName, usar ese
    if (props.botonName) {
        return props.botonName;
    }

    // PRIORIDAD 4: Si viene soli, usar ese
    if (props.soli) {
        return props.soli;
    }

    // PRIORIDAD 5: Si hay datos existentes
