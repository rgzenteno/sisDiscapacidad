// ============ INICIO IMPORTS ============ //
import { checkDistritoDuplicate, checkDiscapacidadDuplicate } from '../utils/duplicateCheck';
// ============ FIN IMPORTS ============ //
...
export function useFormValidation(form, props, emit) {
    ...
    const validarDuplicados = () => {
        ...
        if (props.submitRoute === 'dropdown.store' && form.distrito) {
            const existe = checkDistritoDuplicate(form.distrito, props.distritos);
            if (existe) {
                emit('encontrado');
                form.reset();
                return false;
        ...
        if (props.submitRoute === 'dropdown.addDis' && form.discapacidad) {
            const existe = checkDiscapacidadDuplicate(form.discapacidad, props.discapacidad);
            if (existe) {
                emit('encontrado');
                form.reset();
                return false;
    ...
    const validateForm = (fileValidation) => {
        ...
        if (props.gestion && props.gestion.length > 0) {
            const mostrarPresupuestoAnual = !props.dataGestion ||
                !props.dataGestion.some(g => g.gestion === props.gestion.gestion);

            if (mostrarPresupuestoAnual && (!form.presupuesto_anual || form.presupuesto_anual.trim() === '')) {
